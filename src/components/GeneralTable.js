...
import {shipmentDeleted, dateSortedAscending, dateSortedDescending, customerSortedAscending, customerSortedDescending } from "../redux/shipmentsSlice/shipmentsActions";
...
export default function GeneralTable({ handleEdit }) {
  ...
  function handleCustomerSort() {
    if(customerSorted) {
      dispatch(customerSortedDescending());
      setCustomerSorted(false);
    } else {
      dispatch(customerSortedAscending());
  ...
  function handleDateSort() {
    if(dateSorted) {
      dispatch(dateSortedDescending());
      setDateSorted(false);
    } else {
      dispatch (dateSortedAscending());
      setDateSorted(true);
    }
  }
                    ...
                    <div className="button-block">
                      ...
                      <Stack spacing={1} direction="row">
                        <IconButton
                            aria-label="delete"
                            id={item.trackingNo}
                            onClick={()=>dispatch(shipmentDeleted(item))}>
